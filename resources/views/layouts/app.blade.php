...
<body class="bg-surface text-on-surface font-body antialiased min-h-screen flex flex-col selection:bg-primary-container selection:text-on-primary-container">
    ...
    <header class="bg-surface/80 backdrop-blur-md shadow-sm sticky top-0 z-50 transition-all duration-500 ease-in-out" id="main-nav">
            ...
            <div class="hidden md:flex items-center gap-md">
                <div class="relative group">
                    ...
                    <div class="absolute top-full right-0 mt-2 w-56 bg-surface-container-lowest rounded-2xl card-shadow opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform origin-top-right scale-95 group-hover:scale-100 z-50 overflow-hidden">
                        <a href="{{ route('landing') }}" onclick="document.getElementById('menuLabel').querySelector('span').textContent='Beranda'"
                        ...
                        <a href="{{ route('tiket.index') }}" onclick="document.getElementById('menuLabel').querySelector('span').textContent='Tiket'"
                        ...
                        @auth
                        <a href="{{ route('wisatawan.pemesanan.riwayat') }}" onclick="document.getElementById('menuLabel').querySelector('span').textContent='Riwayat'"
                ...
                @auth
                <form id="logoutFormDesktop" method="POST" action="{{ route('logout') }}" style="display:none">
                    @csrf
                </form>
                <button onclick="document.getElementById('logoutFormDesktop').submit()" class="font-body text-label-md text-primary hover:bg-primary/5 transition-colors px-4 py-2 rounded-xl">Logout</button>
                @else
        ...
        <div id="mobileMenu" class="hidden md:hidden bg-surface border-t border-outline-variant/20 px-gutter py-md space-y-3">
            <a href="{{ route('landing') }}" class="block font-body text-body-md text-on-surface-variant hover:text-primary">Beranda</a>
            <a href="#lokasi" class="block font-body text-body-md text-on-surface-variant hover:text-primary">Lokasi</a>
            <a href="{{ route('tiket.index') }}" class="block font-body text-body-md text-on-surface-variant hover:text-primary">Tiket</a>
            @auth
            <a href="{{ route('wisatawan.pemesanan.riwayat') }}" class="block font-body text-body-md text-on-surface-variant hover:text-primary">Riwayat</a>
            <form id="logoutFormMobile" method="POST" action="{{ route('logout') }}" style="display:none">
                @csrf
            </form>
            <button onclick="document.getElementById('logoutFormMobile').submit()" class="block font-body text-body-md text-primary">Logout</button>
            @else
            <a href="javascript:void(0)" onclick="document.getElementById('loginModal').classList.remove('hidden')" class="block font-body text-body-md text-on-surface-variant hover:text-primary">Riwayat</a>
            <a href="{{ route('login') }}" class="block font-body text-body-md text-primary">Login</a>
            <a href="{{ route('register') }}" class="block font-body text-body-md text-primary">Register</a>
            @endauth
        </div>
    </header>
    ...
    <div id="loginModal" class="hidden fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm">
        <div class="bg-surface-container-lowest rounded-2xl card-shadow p-xl max-w-sm mx-4 text-center transform scale-95 animate-modal-in">
            ...
            <h3 class="font-display text-headline-sm text-on-background mb-sm">Login Diperlukan</h3>
            <p class="font-body text-body-md text-on-surface-variant mb-lg">Silakan login atau daftar untuk melihat riwayat booking Anda.</p>
            <div class="flex flex-col sm:flex-row gap-sm justify-center">
                <a href="{{ route('login') }}"
                   class="bg-primary-container text-white font-body text-label-md px-6 py-3 rounded-xl hover:-translate-y-0.5 transition-all">Login</a>
                <a href="{{ route('register') }}"
                   class="border border-primary text-primary font-body text-label-md px-6 py-3 rounded-xl hover:bg-primary/5 transition-all">Daftar</a>
            </div>
            <button onclick="document.getElementById('loginModal').classList.add('hidden')"
                    class="mt-md font-body text-body-md text-outline hover:text-on-surface-variant transition-colors">Nanti Saja</button>
        </div>
    </div>
    ...
    <footer id="footer" class="bg-surface-container-low dark:bg-surface-container-lowest border-t border-outline-variant/20">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-lg px-gutter py-xl max-w-container-max mx-auto w-full">
            <div class="md:col-span-2">
                ...
                <p class="font-body text-body-md text-on-surface-variant mb-md max-w-md">Tropical Elegance in Bali. Discover the unseen beauty and tranquility of Gianyar's hidden gem.</p>
                <p class="font-body text-caption text-outline">&copy; {{ date('Y') }} Bangkiang Jaran Waterfall. Tropical Elegance in Bali.</p>
            </div>
            <div>
                <h3 class="font-body text-label-md text-primary mb-sm uppercase tracking-wider">Navigasi</h3>
                <ul class="space-y-sm">
                    <li><a href="{{ route('landing') }}" class="font-body text-body-md text-outline hover:text-primary transition-colors">Beranda</a></li>
                    <li><a href="#lokasi" class="font-body text-body-md text-outline hover:text-primary transition-colors">Lokasi</a></li>
                    <li><a href="{{ route('tiket.index') }}" class="font-body text-body-md text-outline hover:text-primary transition-colors">Tiket</a></li>
                </ul>
            </div>
            <div>
                <h3 class="font-body text-label-md text-primary mb-sm uppercase tracking-wider">Kontak</h3>
                <ul class="space-y-sm">
    ...
    <script>
        ...
        (function() {
            ...
            if (btn) {
                ...
                if (path === '/' || path === '/landing') span.textContent = 'Beranda';
                else if (path.startsWith('/tiket')) span.textContent = 'Tiket';
                else if (path.startsWith('/wisatawan/pemesanan/riwayat')) span.textContent = 'Riwayat';
            }
        })();
