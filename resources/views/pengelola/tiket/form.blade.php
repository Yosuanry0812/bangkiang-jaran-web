...
@section('title', isset($tiket) ? 'Edit Tiket' : 'Tambah Tiket')

@section('content')
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="font-heading text-2xl font-bold text-stone-800">{{ isset($tiket) ? 'Edit Tiket' : 'Tambah Tiket' }}</h1>
            <p class="text-sm text-stone-500 mt-0.5">{{ isset($tiket) ? 'Ubah informasi tiket' : 'Buat tiket baru' }}</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm p-6 max-w-2xl">
        <form action="{{ isset($tiket) ? route('pengelola.tiket.update', $tiket->id_tiket) : route('pengelola.tiket.store') }}" method="POST">
            ...
            <div class="space-y-5">
                <div>
                    <label class="block text-xs font-semibold uppercase tracking-wider text-stone-500 mb-1.5">Nama Tiket</label>
                ...
                <div>
                    <label class="block text-xs font-semibold uppercase tracking-wider text-stone-500 mb-1.5">Harga (Rp)</label>
                ...
                <div>
                    <label class="block text-xs font-semibold uppercase tracking-wider text-stone-500 mb-1.5">Status</label>
                    <select name="status" required
                        ...
                        <option value="aktif" {{ old('status', $tiket->status ?? '') == 'aktif' ? 'selected' : '' }}>Aktif</option>
                        <option value="nonaktif" {{ old('status', $tiket->status ?? '') == 'nonaktif' ? 'selected' : '' }}>Nonaktif</option>
                    </select>
