...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="font-heading text-2xl font-bold text-gray-500-800">{{ __('messages.manage_tickets_title') }}</h1>
            <p class="text-sm text-gray-500-500 mt-0.5">{{ __('messages.manage_tickets_desc') }}</p>
        </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-gray-300-200 bg-stone-50 text-left">
                        <th class="text-xs font-semibold uppercase tracking-wider text-gray-500-500 py-3.5 px-4">{{ __('messages.th_ticket_name') }}</th>
                        <th class="text-xs font-semibold uppercase tracking-wider text-gray-500-500 py-3.5 px-4">{{ __('messages.th_price') }}</th>
                        <th class="text-xs font-semibold uppercase tracking-wider text-gray-500-500 py-3.5 px-4">{{ __('messages.th_total_sold') }}</th>
                ...
                <tbody>
                    ...
                    <tr class="border-b border-gray-300-100 hover:bg-stone-50 transition-colors">
                        <td class="py-3.5 px-4 font-medium text-gray-500-800">{{ $t->nama_tiket }}</td>
                        <td class="py-3.5 px-4 text-gray-500-700">Rp {{ number_format($t->harga, 0, ',', '.') }}</td>
                        <td class="py-3.5 px-4 text-gray-500-600">{{ $t->total_terjual ?? 0 }}</td>
                    ...
                    @empty
                    <tr><td colspan="5" class="py-10 text-center text-gray-500-400">{{ __('messages.no_tickets_yet') }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
