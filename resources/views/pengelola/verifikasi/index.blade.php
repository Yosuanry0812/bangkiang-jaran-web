...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="font-heading text-2xl font-bold text-gray-500-800">{{ __('messages.verification_title') }}</h1>
            <p class="text-sm text-gray-500-500 mt-0.5">{{ __('messages.verification_desc') }}</p>
        </div>
    </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-gray-300-200 bg-stone-50 text-left">
                        ...
                        <th class="text-xs font-semibold uppercase tracking-wider text-gray-500-500 py-3.5 px-4">{{ __('messages.th_payment_status') }}</th>
                ...
                <tbody>
                    ...
                    <tr class="border-b border-gray-300-100 hover:bg-stone-50 transition-colors">
                        <td class="py-3.5 px-4 font-mono text-gray-500-700">{{ $p->kode_booking }}</td>
                        <td class="py-3.5 px-4 text-gray-500-600">{{ $p->user->name ?? '-' }}</td>
                        <td class="py-3.5 px-4 text-gray-500-600">
                            @php
                                $tSummary = $p->detailPemesanan->groupBy('nama_tiket')->map(fn($g) => $g->count() . '× ' . $g->first()->nama_tiket)->implode('<br>');
                            @endphp
                            {!! $tSummary ?: ($p->tiket->nama_tiket ?? '-') !!}
                        </td>
                        <td class="py-3.5 px-4 text-gray-500-500 text-xs">{{ $p->tgl_kunjungan ? \Carbon\Carbon::parse($p->tgl_kunjungan)->format('d/m/Y') : '-' }}</td>
                        <td class="py-3.5 px-4 text-gray-500-700 font-medium">Rp {{ number_format($p->total_harga, 0, ',', '.') }}</td>
                    ...
                    @empty
                    <tr><td colspan="8" class="py-10 text-center text-gray-500-400">{{ __('messages.no_verification_data') }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
